...
export class TasksService 
{
    ...
    {
        const taskBoard = this.taskBoardRepository.create({ serviceID });
    ...
    async findTaskBoardById(id: number): Promise<TaskBoard | null>
    {
        return await this.taskBoardRepository.findOne({
            where: { serviceID: id }, // Adjust to match your primary key
            relations: [ 'cards' ]
        });
    }
    ...
    {
        await this.taskBoardRepository.update(id, { serviceID });
    ...
    async createCard(taskBoardId: number, column: string, title: string, description: string): Promise<Card>
    {
        ...
        const taskBoard = await this.taskBoardRepository.findOne(taskBoardId);
    ...
    {
        return await this.cardRepository.find({ where: { taskBoard: { serviceID: taskBoardId } } });
    }
}
